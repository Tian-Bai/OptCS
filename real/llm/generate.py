...
sequences = []
bs = 1
for i in range((args.idx-1)*args.batch_size, args.idx*args.batch_size, bs):
    print(f"idx: {i}")
    ...
    # remove "2.0.0/files"
    # print(os.path.join('image', test_sub['image_path'][0][len("2.0.0/files/"):]))
    images_sub = [read_image(os.path.join('image', image_path[len("2.0.0/files/"):]), mode=ImageReadMode.RGB) for image_path in test_sub['image_path']]
