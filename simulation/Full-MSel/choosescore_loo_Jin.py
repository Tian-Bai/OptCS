...
def Split_train_modsel(mdl_idx, Xtotal, Ytotal, split_ratio, Xtest):
    ...
    if mdl_idx < 3: 
        qt = 0.25 * (mdl_idx + 1)
        mdl = RandomForestQuantileRegressor(max_depth=5, default_quantiles=qt, random_state=0)
    elif mdl_idx in [3, 6]:
        mdl = SVR(kernel='rbf', gamma=0.1)
    elif mdl_idx in [4, 7]:
        mdl = Lasso(alpha=1)
    elif mdl_idx in [5, 8]:
        mdl = Ridge(alpha=1)
    ...
    pred_test = mdl.predict(Xtest) / (1 if isinstance(denom, int) else denom.predict(Xtest))
    pred_calib = mdl.predict(Xcalib) / (1 if isinstance(denom, int) else denom.predict(Xcalib))
...
def Modsel_train_modsel(Xtotal, Ytotal, split_ratio1, split_ratio2, Xtest, q_list):
    ...
    for mdl_idx in range(9): # modsel = 9
        if mdl_idx < 3: 
            qt = 0.25 * (mdl_idx + 1)
            mdl = RandomForestQuantileRegressor(max_depth=5, default_quantiles=qt, random_state=0)
        elif mdl_idx in [3, 6]:
    ...
    try_selections = np.zeros((len(q_list), 9))
    ...
    for qid, q in enumerate(q_list):
        ...
        theta_j = np.random.choice([a for a in range(9) if try_selections[qid][a] == max_sel])
...
for seed in range(Nrep * seedgroup, Nrep * (seedgroup+1)):
    random.seed(seed)
    np.random.seed(seed)
...
all_res.to_csv(os.path.join('results', f"choosescore_LOO_Jin, setting={setting}, dim={dim}, sigma={sig}, seedgroup={seedgroup}, ntest={ntest}, nlabel={nlabel}.csv"))