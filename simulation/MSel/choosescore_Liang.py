...
from sklearn.linear_model import QuantileRegressor, Ridge, Lasso, LinearRegression
...
for i_itr in range(Nrep * seedgroup, Nrep * (seedgroup+1)):
    ...
    Xtrain, Ytrain = gen_data_Liang2024(setting, ntrain, sig, dim)
    Xcalib, Ycalib = gen_data_Liang2024(setting, ncalib, sig, dim)
    ...
    # 9 conditional quantile regressors: linear
    for qq in range(1, 10):
        features = np.random.choice(dim, dim // 10)
        qt = qq / 10 # quantile level
        linear_qt = QuantileRegressor(quantile=qt)
        linear_qt.fit(Xtrain[:, features], Ytrain)
        features_list.append(features)
        model_list.append(linear_qt)
        denom_list.append(1)

    features = np.random.choice(dim, dim // 5)
    linear = LinearRegression()
    linear.fit(Xtrain[:, features], Ytrain)

    Xsigma, Ysigma = gen_data_Liang2024(setting, 100, sig, dim)
    linear_sigma = LinearRegression()
    linear_sigma.fit(Xsigma, np.absolute(Ysigma - linear.predict(Xsigma[:, features])))

    model_list += [linear] * 2
    features_list += [features] * 2
    denom_list += [1, linear_sigma]
    ...
    features = features_list[model_idx]

    calib_scores = 1000 * (Ycalib > 0) - reg.predict(Xcalib[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xcalib))
    test_scores = -reg.predict(Xtest[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xtest))
    ...
    for k in range(modelnum):
        ...
        calib_scores_modsel = 1000 * (Ymodsel_calib > 0) - reg.predict(Xmodelsel_calib[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xmodelsel_calib))
        test_scores_modsel = -reg.predict(Xmodsel_test[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xmodsel_test))
    ...
    for qid, q in enumerate(q_list):
        ...
        calib_scores = 1000 * (Ycalib_0 > 0) - reg.predict(Xcalib_0[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xcalib_0))
        test_scores = -reg.predict(Xtest[:, features]) / (1 if isinstance(denom, int) else denom.predict(Xtest))
    ...
    for qq in range(1, 10):
        ...
        qt = qq / 10 # quantile level
        linear_qt = QuantileRegressor(quantile=qt)
        linear_qt.fit(Xtrain_0[:, features], Ytrain_0)
        features_list.append(features)
        model_list.append(linear_qt)
        denom_list.append(1)
    ...
    features = np.random.choice(dim, dim // 5)
    linear = LinearRegression()
    linear.fit(Xtrain_0[:, features], Ytrain_0)
...
all_res.to_csv(os.path.join('results', f"Liang, setting={setting}, modelnum={modelnum}, dim={dim}, sigma={sig}, ntest={ntest}, ncalib={ncalib}, ntrain={ntrain}, seed={seedgroup}.csv"))